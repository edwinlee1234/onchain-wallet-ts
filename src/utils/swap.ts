...
async function getSwapRoute(
  ...
  const { data: route } = await axios.get<RouteResponse>(quoteUrl);
...
async function getTxStatus(hash: string, lastValidBlockHeight: number): Promise<txStatusRes> {
  ...
  const { data: status } = await axios.get<txStatusRes>(statusUrl);
...
async function sendTx(signedTx: string): Promise<SendTxRes> {
  const response = await axios.post<SendTxRes>(
    `${API_HOST}/txproxy/v1/send_transaction`,
    {
      chain: 'sol',
      signedTx: signedTx
    },
